...
<style>
 ...
 setTimeout(() => {
  ...
  aprobarReceta(id: string | undefined): void {
    if (!id) return;
    this.recetasService.actualizarEstadoReceta(id, 'aprobada').subscribe({
      next: () => {
  ...
  confirmarRechazoReceta(): void {
    if (!this.recetaParaRechazar?.id || !this.motivoRechazo.trim()) return;

    this.recetasService.actualizarEstadoReceta(this.recetaParaRechazar.id, 'rechazada').subscribe({
      next: () => {
